...
def anal_string(n_chains, n_iterations, rate_hdi, sim_passes):
    anal_string = """
    The model was fit to the the data for each campaign, where each question asked and number of responses requested condition corresponded to a distinct campaign.
...
data {
    ...
    int <lower=0, upper=1> novel[N]; // whether there was a novel category at this point
...
def plot_model_per_question(df, n_iter, n_chains, init_params):
    ...
    fig.savefig('figures/rate_model_cat_questions', dpi=600)
    #plt.show()
...
def plot_model(rate, min_rate, df, field):
    ...
    fig.savefig('figures/bernoulli_decay_model_cat_example', dpi=600)
    #plt.show()
...
if __name__ == '__main__':
    ...
    with open('tex/bernoulli_decay_model_cat_anal.tex', 'w') as f:
