...
# Various NLP processing things.
# Bastardized to work in my Python3 env
...
def bag_of_words_cos_similarity_weighted(l, r, fd):
  if len(l) == 0 or len(r) == 0:
    return 0
  common_set = l.intersection(r)
  total_set = l.union(r)
  ...
  common_weights = sum(1 / fd.freq(w) for w in common_set)
  #total_weights = sum(fd.freq(w) for w in total_set)

  return common_weights / denom
...
def extract_features(sentence, correct_spelling, include_bigrams=False, include_trigrams=False, filter_stopwords=False, include_nyms=True):
  ...
  lower_tokens = [w.lower() for w, tag in pos_tags]
